...
bool existOffers(const char* filePath, const char* companyName) {
	...
	while (!inFile.eof())	{
		inFile.read((char*)&currentOffer, sizeof(JobOffer));
		if (!strcmp(companyName, currentOffer.companyName)) {
			return true;
		}
	}
	return false;
...
int main() {
	...
	std::cout << sizeof(Structy);
	//JobOffer j1;
	//strcpy(j1.companyName, "companyName5");
	//j1.countCoWorkers = 5;
	//j1.freeDays = 30;
	//j1.salary = 1800;

	//JobOffer j2;
	//strcpy(j2.companyName, "companyName6");
